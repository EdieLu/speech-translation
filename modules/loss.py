...
class KLDivLoss(Loss):
	...
	def eval_batch_with_mask(self, logq, logp, mask):
		p = torch.exp(logp)
		masked_loss = self.criterion(logq, p).sum(dim=1).masked_select(mask)
		self.acc_loss += masked_loss.sum()
...
class MSELoss(Loss):
	...
	def eval_batch(self, input, target):
		self.acc_loss += torch.sum(self.criterion(input, target))
