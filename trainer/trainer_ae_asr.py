			...
			for idx in range(len(evaliter)):
				...
				for bidx in range(n_minibatch):
					...
					out_count_dummy = self._print(out_count, src_ids,
						dataset.src_id2word, seqres_asr, tail='-asr')
					out_count = self._print(out_count, refs_ae_add,
						dataset.src_id2word, seqres_ae, tail='-ae ')

					# accumulate corpus
					hyp_corpus_asr, ref_corpus_asr = add2corpus(seqres_asr, src_ids,
						dataset.src_id2word, hyp_corpus_asr, ref_corpus_asr, type='word')
					hyp_corpus_ae, ref_corpus_ae = add2corpus(seqres_ae, refs_ae_add,
						dataset.src_id2word, hyp_corpus_ae, ref_corpus_ae, type='word')
		...
		for epoch in range(start_epoch, n_epochs + 1):
			...
			for idx in range(steps_per_epoch):
				...
				if step % self.print_every == 0 and step_elapsed > self.print_every:
					...
					log_msg += 'Train l2: %.4f, kl_en: %.4f' % (
						print_loss_l2_avg, print_loss_kl_avg)
					log.info(log_msg)
				...
				if step % self.checkpoint_every == 0 or step == total_steps:
					...
					if dev_set is not None:
						...
						# save criteria
						accuracy_ave = (accuracy_asr + accuracy_ae)/2.0
						bleu_ave = (bleu_asr + bleu_ae)/2.0
						if loss_kl > 0:
							# use kl
							save_criteria = (prev_kl > loss_kl) and (bleu_asr > 0.5)
						else:
							# use ave bleu of (asr + ae)
							save_criteria = ((prev_acc < accuracy_ave) and (bleu_ave < 0.1)) or prev_bleu < bleu_ave
