...
class Trainer_ASR_ST(Trainer):
	...
	def _evaluate_batches(self, model, dataset):
		...
		with torch.no_grad():
			for idx in range(len(evaliter)):
				...
				for bidx in range(n_minibatch):
					...
					#	acous_lens=acous_lengths, mode='ASR_ST', use_gpu=self.use_gpu)
		...
		resloss_en *= self.loss_coeff['nll_asr']
	...
	def _train_batch(self,
		...
		for bidx in range(n_minibatch):
			...
			loss_en.acc_loss *= self.loss_coeff['nll_asr']
