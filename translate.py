...
def translate(test_set, model, test_path_out, use_gpu,
	max_seq_len, beam_width, device, seqrev=False, gen_mode='ASR'):
	...
	# reset max_len
	if 'ASR' in model.mode or 'ST' in model.mode:
		model.las.decoder.max_seq_len = 150
	if 'MT' in model.mode:
		model.enc_src.expand_time(150)
	if 'ST' in model.mode or 'MT' in model.mode:
...
def main():
	...
	if MODE == 1:
		translate(test_set, model, test_path_out, use_gpu,
			max_seq_len, beam_width, device, seqrev=seqrev, gen_mode=config['gen_mode'])
